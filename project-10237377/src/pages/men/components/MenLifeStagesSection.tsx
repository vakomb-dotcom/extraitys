...
const menProducts = allProducts.filter((p) => p.category === 'MEN').slice(0, 20);

export default function MenLifeStagesSection() {
    ...
    <section ref={sectionRef} className="w-full py-8 md:py-12" style={{ background: '#F5F0EB' }}>
      <div className="px-5 md:px-12 lg:px-20 mb-8">
        <div className={`transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-8'}`}>
          <p className="text-[#9B2226]/50 text-xs tracking-[0.35em] uppercase mb-2">For him</p>
          <h2 className="serif text-[#1A1A1A] leading-none italic font-semibold" style={{ fontSize: 'clamp(32px, 4.5vw, 56px)' }}>
            MEN'S <span style={{ color: '#9B2226' }}>COLLECTION</span>
          </h2>
        </div>
      </div>
