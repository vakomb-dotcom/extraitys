...
export default function WomenCtaSection() {
  ...
  return (
    <section ref={ref} className="w-full relative overflow-hidden" style={{ background: '#1A1015', minHeight: '280px' }}>
      <div className="absolute inset-0 opacity-10">
        <img
          src="https://readdy.ai/api/search-image?query=luxury%20perfume%20bottles%20collection%2C%20elegant%20glass%20bottles%20with%20warm%20gold%20accents%2C%20dark%20burgundy%20background%2C%20professional%20product%20photography%2C%20artisan%20perfumery%2C%20sophisticated%20feminine%20display%2C%20dramatic%20soft%20lighting&width=1400&height=400&seq=women-cta-bg-v3&orientation=landscape"
          alt="Luxury perfume collection"
          className="w-full h-full object-cover object-top"
        />
      </div>
      <div className="absolute inset-0 bg-gradient-to-r from-[#1A1015] via-[#1A1015]/95 to-[#1A1015]/70" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between px-5 md:px-12 lg:px-20 py-14 gap-8">
        <div className={`transition-all duration-800 ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
          <h2 className="serif text-white leading-none italic font-semibold" style={{ fontSize: 'clamp(28px, 4.5vw, 50px)' }}>
            YOUR SCENT<br />
            <span style={{ color: '#C4A44A' }}>YOUR STORY</span>
          </h2>
        </div>
